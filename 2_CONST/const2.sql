...
create table CONST_DEPT(
   DEPTNO number(2) constraint CONST_DEPT_PK primary key, 
   DNAME varchar2(14) default '개발부', 
   LOC char(1) constraint CONST_DEPT_CK check(LOC in('1','2'))  
);
create table CONST_EMP(
   NO number(4) constraint CONST_EMP_PK primary key, 
   NAME varchar2(10) constraint CONST_EMP_NNN not null, 
   ADDR varchar2(6) constraint CONST_EMP_CK check(ADDR in('서울','부산')),
   JUMIN char(13) constraint CONST_EMP_UQ unique constraint CONST_EMP_JNN not null, 
   RDATE date default SYSDATE, 
   DEPTNO number(2) constraint CONST_EMP_FK references CONST_DEPT(DEPTNO)   
);
